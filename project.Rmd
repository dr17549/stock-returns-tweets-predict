...
```{r}
df_emoji_text <- replace_emoji(df$tweet)

df_emoji_identifier <- replace_emoji(df$tweet) 
```


```{r}
replaced_text <- replace_emoji("hello 😠")
```
...
dfm_faang_test_pros<-TMEF_dfm(faang_test_data$tweet,
                               ngrams=1:2,
                               min.prop = 0) %>%
...
acc <- kendall_acc(faang_test_predict_pros,test_Y)
