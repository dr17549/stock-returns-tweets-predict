...
Save and load the DFM as an RDS file 
```{r}
...
print(paste("Number of rows:", nrow(faang_small)))
...
dfm_faang_train_pros <-custom_dfm(my_data_sample$tweet,
                                  ngrams=1:2,
                                  custom_stop_words = 
                                    c("amazon","facebook","google")) %>%
  convert(to="matrix")
...
# Random Forest 
!! does not work yet ? seems to be a problem with the emoji 
```{r}
# Change format of the dataset so it works with the naive bayes function 
naive_bayes_train <- as.data.frame(cbind(dfm_faang_train_pros, faang_train_Y))
naive_bayes_train$faang_train_Y <- as.factor(df_from_array$faang_train_Y)
head(naive_bayes_train)
faang_model_pros <- randomForest(faang_train_Y ~ ., data = naive_bayes_train)
